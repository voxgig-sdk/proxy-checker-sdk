...
describe('IpInformationDirect', async () => {
  ...
  test('direct-load-ip_information', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ip_information', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PROXY_CHECKER_TEST_IP_INFORMATION_ENTID']
