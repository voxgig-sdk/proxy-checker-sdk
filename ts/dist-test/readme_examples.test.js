...
const __1 = require("..");
const SDK_NAME = 'ProxyCheckerSDK';
...
const TEST_SEED = { "entity": { "check": { "test01": { "id": "test01" } }, "ip_information": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ProxyCheckerSDK\b/.test(code) ||
        /\bProxyCheckerSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ProxyCheckerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ProxyCheckerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
