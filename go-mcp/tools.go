...
type Args struct {
	Entity string         `json:"entity" jsonschema:"check | ip_information"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ProxyCheckerSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "proxy-checker_list",
		Description: "List records from ProxyChecker. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "proxy-checker_load",
		Description: "Load a single record from ProxyChecker. " +
...
func runOp(client *sdk.ProxyCheckerSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ProxyCheckerSDK, name string) (sdk.ProxyCheckerEntity, error) {
	switch strings.ToLower(name) {
	case "check":
		return client.Check(nil), nil
	case "ip_information":
		return client.IpInformation(nil), nil
