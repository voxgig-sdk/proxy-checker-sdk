...
[MCP](https://modelcontextprotocol.io) server exposing the ProxyChecker SDK as
two agent tools — `proxy-checker_list` and `proxy-checker_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/proxy-checker-mcp)
make build

# 2. Provide credentials via the environment
export PROXY_CHECKER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user proxy-checker \
  -- /absolute/path/to/proxy-checker-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./proxy-checker-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// proxy-checker_list: first page of records
{ "entity": "check" }
{ "entity": "check", "query": { } }

// proxy-checker_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/proxy-checker-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user proxy-checker \
     -- "$PWD"/dist/*/proxy-checker-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `proxy-checker_list` and `proxy-checker_load` tools now appear
   in new sessions. Ask the agent to *"list check using proxy-checker"*
   and it calls `proxy-checker_list` with `{"entity":"check"}`.
...
```sh
export PROXY_CHECKER_APIKEY=sk_live_xxx            # API key
export PROXY_CHECKER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `proxy-checker_list` tool
...
### Call the `proxy-checker_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `proxy-checker_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `proxy-checker_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PROXY_CHECKER_APIKEY` | API key sent with every request. |
| `PROXY_CHECKER_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

check | ip_information

### Smoke test via HTTP (raw JSON-RPC)

```sh
./proxy-checker-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"proxy-checker_load","arguments":{"entity":"check","query":{"id":1}}}}'
```
