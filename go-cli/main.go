// Command ProxyChecker-cli is a CLI + REPL for the ProxyChecker SDK,
...
const prompt = "proxy-checker"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "check ip_information"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PROXY_CHECKER_APIKEY (api key), PROXY_CHECKER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PROXY_CHECKER_APIKEY carries the API key and
	// PROXY_CHECKER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PROXY_CHECKER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PROXY_CHECKER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewProxyCheckerSDK(opts)
