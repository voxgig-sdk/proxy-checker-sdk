...
boru-driven command-line client **and** interactive REPL for the ProxyChecker
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/proxy-checker-cli)
make build

# 2. See usage (words, entities, env vars)
./proxy-checker-cli --help

# 3. Provide credentials once, via the environment
export PROXY_CHECKER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./proxy-checker-cli load 1 check            # {id:1} shorthand
./proxy-checker-cli load '{id:1}' check       # explicit match map

# 5. Override the API base URL for a single call
PROXY_CHECKER_BASE=https://api.example.com ./proxy-checker-cli load 1 check

# 6. No arguments -> interactive REPL
./proxy-checker-cli
proxy-checker> load 1 check
proxy-checker> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/proxy-checker-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/proxy-checker-cli load 1 check
   ```
...
```sh
./proxy-checker-cli load 1 check          # scalar shorthand for {id:1}
./proxy-checker-cli load '{id:1}' check     # explicit match map
```
...
./proxy-checker-cli load 1 check
```
...
Run with no arguments to open a REPL (prompt `proxy-checker>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/proxy-checker-cli`. |
...
(`./proxy-checker-cli <expr>`) and interactively (the REPL), and expressions compose the
