...
local ProxyCheckerSDK = {}
ProxyCheckerSDK.__index = ProxyCheckerSDK
...
ProxyCheckerSDK._make_feature = _make_feature


function ProxyCheckerSDK.new(options)
  local self = setmetatable({}, ProxyCheckerSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ProxyCheckerSDK:options_map()
...
function ProxyCheckerSDK:get_utility()
  return Utility.copy(self._utility)
end


function ProxyCheckerSDK:get_root_ctx()
  return self._rootctx
end


function ProxyCheckerSDK:prepare(fetchargs)
...
function ProxyCheckerSDK:direct(fetchargs)
...
-- by passing it.
function ProxyCheckerSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ProxyCheckerSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ProxyCheckerSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ProxyCheckerSDK:Check(data)
...
-- Idiomatic facade: client:IpInformation():list() / client:IpInformation():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function ProxyCheckerSDK:IpInformation(data)
  local EntityMod = require("entity.ip_information_entity")
  if data == nil then
    if self._ip_information == nil then
      self._ip_information = EntityMod.new(self, nil)
    end
    return self._ip_information
  end
  return EntityMod.new(self, data)
end
...
function ProxyCheckerSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ProxyCheckerSDK.new(sdkopts)
  sdk.mode = "test"
