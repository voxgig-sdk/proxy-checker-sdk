...
local IpInformationEntity = {}
IpInformationEntity.__index = IpInformationEntity


function IpInformationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IpInformationEntity)
  self._name = "ip_information"
  self._client = client
...
function IpInformationEntity:get_name()
  return self._name
end


function IpInformationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IpInformationEntity.new(self._client, opts)
end
...
function IpInformationEntity:mark_deleted()
  self._deleted = true
end


function IpInformationEntity:deleted()
  return true == self._deleted
end


function IpInformationEntity:data_set(args)
  if args ~= nil then
...
function IpInformationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IpInformationEntity:match_set(args)
  if args ~= nil then
...
function IpInformationEntity:match_get()
...
function IpInformationEntity:stream(action, args, callopts)
...
---@param reqmatch IpInformationLoadMatch
---@param ctrl? table
---@return IpInformation
---@return string? err
function IpInformationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function IpInformationEntity:_run_op(ctx, post_done)
  local utility = self._utility
