...
class ProxyCheckerTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ProxyCheckerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ProxyCheckerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ProxyCheckerTestRunner.getenv("PROXY_CHECKER_TEST_LIVE")
        override = ProxyCheckerTestRunner.getenv("PROXY_CHECKER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ProxyCheckerTestRunner.getenv(key)
        ...
        explain = ProxyCheckerTestRunner.getenv("PROXY_CHECKER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ProxyCheckerTestRunner._test_control is not None:
            return ProxyCheckerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ProxyCheckerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ProxyCheckerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ProxyCheckerTestRunner.load_test_control()
...
def load_env_local():
    ProxyCheckerTestRunner.load_env_local()


def env_override(m):
    return ProxyCheckerTestRunner.env_override(m)


def entity_data(v):
    return ProxyCheckerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ProxyCheckerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ProxyCheckerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ProxyCheckerTestRunner.load_test_control()


def live_client_options():
    return ProxyCheckerTestRunner.live_client_options()


def live_delay_ms():
    return ProxyCheckerTestRunner.live_delay_ms()
