    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PROXYCHECKER_TEST_CHECK_ENTID JSON to run live")
        client = setup["client"]
        ...
        check_ref01_data = helpers.to_map(check_ref01_ent.create(check_ref01_data, None))
...
def _check_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PROXYCHECKER_TEST_CHECK_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("PROXYCHECKER_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("PROXYCHECKER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("PROXYCHECKER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
