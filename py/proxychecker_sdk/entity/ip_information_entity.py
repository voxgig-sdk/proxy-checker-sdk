...
from proxychecker_sdk.proxychecker_types import (
    IpInformation,
...
class IpInformationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IpInformationEntity(self._client, opts)
    ...
    def data_get(self) -> IpInformation:
    ...
    def match_get(self) -> IpInformation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> IpInformation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.IpInformation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
