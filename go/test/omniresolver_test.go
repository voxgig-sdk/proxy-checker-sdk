...
import (
	...
	omni "github.com/voxgig-sdk/proxy-checker-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ProxyCheckerError); is {
				out := map[string]any{
					"name":    "ProxyCheckerError",
...
func omniCtx(arg any, client *sdk.ProxyCheckerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ProxyCheckerSDK); is {
			client = live
