...
func TestCheckEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PROXYCHECKER_TEST_CHECK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		checkRef01Data = core.ToMapAny(checkRef01DataResult)
...
func checkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PROXYCHECKER_TEST_CHECK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PROXYCHECKER_TEST_CHECK_ENTID"])
