...
// ProxyChecker SDK IpInformation entity
...
class IpInformationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new IpInformationEntity($this->_client, $opts);
    }

    /**
     * @param IpInformation|array $args IpInformation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ProxyCheckerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return IpInformation|array The current IpInformation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of IpInformation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ProxyCheckerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of IpInformation fields).
     */
    ...
    /**
     * Load a single IpInformation.
     *
     * @param IpInformationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed IpInformationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return IpInformation|array The loaded IpInformation as an assoc-array at the
     *   SDK boundary; throws ProxyCheckerError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ProxyCheckerHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
