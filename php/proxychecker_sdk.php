...
class ProxyCheckerSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ProxyCheckerHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ProxyCheckerHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ProxyCheckerFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ProxyCheckerFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ProxyCheckerUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ProxyCheckerHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ProxyCheckerHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ProxyCheckerHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ProxyCheckerError($op . "_allow",
                "ProxyCheckerSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ProxyCheckerHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ProxyCheckerError("graphql_error",
                "ProxyCheckerSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_ip_information = null;

    // Canonical facade: $client->IpInformation()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->ip_information()
    // resolves here too.
    public function IpInformation($data = null)
    {
        require_once __DIR__ . '/entity/ip_information_entity.php';
        if ($data === null) {
            if ($this->_ip_information === null) {
                $this->_ip_information = new IpInformationEntity($this, null);
            }
            return $this->_ip_information;
        }
        return new IpInformationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new ProxyCheckerSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
