...
// ProxyChecker SDK netsim feature
...
class ProxyCheckerNetsimFeature extends ProxyCheckerBaseFeature
{
    ...
    private function _simulate(ProxyCheckerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ProxyCheckerContext $ctx, array $applied): void
    {
        $client = $this->client;
