...
// ProxyChecker SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ProxyCheckerTestFeature extends ProxyCheckerBaseFeature
{
    ...
    public function init(ProxyCheckerContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ProxyCheckerContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ProxyCheckerParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ProxyCheckerContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ProxyCheckerContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ProxyCheckerParam::call($ctx, $k);
