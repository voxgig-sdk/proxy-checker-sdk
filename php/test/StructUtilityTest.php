...
// ProxyChecker SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ProxyCheckerOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ProxyCheckerOmni::structfix($now, Struct::undef()))
                        !== json_encode(ProxyCheckerOmni::structfix($val, Struct::undef()))) {
