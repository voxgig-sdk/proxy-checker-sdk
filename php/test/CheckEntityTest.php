...
class CheckEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PROXYCHECKER_TEST_CHECK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function check_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("PROXYCHECKER_TEST_CHECK_ENTID");
    ...
    if ($env["PROXYCHECKER_TEST_LIVE"] === "TRUE") {
        ...
        $client = new ProxyCheckerSDK(Helpers::to_map($merged_opts));
    }
