...
class IpInformationEntity
  ...
  def make
    opts = @_entopts.dup
    IpInformationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ProxyCheckerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [IpInformation, Hash] the current IpInformation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ProxyCheckerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of IpInformation fields)
  def match_get
  ...
  #
  # @param reqmatch [IpInformationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.IpInformation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [IpInformation, Hash] the loaded IpInformation; raises ProxyCheckerError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ProxyCheckerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
