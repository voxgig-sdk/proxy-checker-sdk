# ProxyChecker Ruby SDK



The Ruby SDK for the ProxyChecker API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Check` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/proxy-checker-sdk/releases](https://github.com/voxgig-sdk/proxy-checker-sdk/releases)
...
```ruby
client = ProxyCheckerSDK.test(testopts, sdkopts)
```
...
| `IpInformation` | `(data) -> IpInformationEntity` | Create an IpInformation entity instance. |
...
raise a `ProxyCheckerError` (a `StandardError` subclass), so wrap
...
Create an instance: `ip_information = client.IpInformation`
...
```
rb/
├── ProxyChecker_sdk.rb       -- Main SDK module
...
The main module (`ProxyChecker_sdk`) exports the SDK class
