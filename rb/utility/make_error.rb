...
module ProxyCheckerUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ProxyCheckerContext.new({}, nil)
    end
    op = ctx.op || ProxyCheckerOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ProxyCheckerResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ProxyCheckerError) ? err.msg : err.to_s
    ...
    sdk_err = ProxyCheckerError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ProxyCheckerError)
