...
require_relative 'ProxyChecker_types'
...
class ProxyCheckerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ProxyCheckerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ProxyCheckerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ProxyCheckerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ProxyCheckerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ProxyCheckerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ProxyCheckerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ProxyCheckerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ProxyCheckerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ProxyCheckerError.new(
        "graphql_error", "ProxyCheckerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.IpInformation.list / client.IpInformation.load({ "id" => ... })
  def IpInformation(data = nil)
    require_relative 'entity/ip_information_entity'
    IpInformationEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ProxyCheckerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
