...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ProxyCheckerOmni.make_runner(TEST_JSON_FILE, ProxyCheckerSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    ProxyCheckerError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ProxyCheckerOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = ProxyCheckerOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
