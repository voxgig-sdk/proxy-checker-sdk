...
module ProxyCheckerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ProxyCheckerResponse.new({})
    ...
    if fetch_err
      ...
      response = ProxyCheckerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ProxyCheckerResponse.new(fetched)
    else
