...
// ProxyChecker SDK retry feature
...
class ProxyCheckerRetryFeature extends ProxyCheckerBaseFeature
{
    ...
    public function init(ProxyCheckerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ProxyCheckerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ProxyCheckerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ProxyCheckerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
