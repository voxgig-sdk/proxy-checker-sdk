...
class FtCtrl extends ProxyCheckerControl
{
...
class FtClient
{
    ...
    private ProxyCheckerUtility $utility;

    public function __construct(ProxyCheckerUtility $utility, array $options)
    {
    ...
    public function get_utility(): ProxyCheckerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ProxyCheckerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ProxyCheckerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ProxyCheckerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ProxyCheckerConfig::make_feature($name);
    ...
    private static function buildUrl(ProxyCheckerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ProxyCheckerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ProxyCheckerResponse($fetched);
        }
    ...
    private function fail(ProxyCheckerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ProxyCheckerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
